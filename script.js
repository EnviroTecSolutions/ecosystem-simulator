let grass, pademelons, devils, chart;
...
let ongoingObjectives = [
  ...
  { description: "Maintain a balanced ecosystem for 2000 frames", target: 2000, achieved: false, points: 500, type: "balanced", condition: (grass, pademelons, devils) => grass > 500 && pademelons > 100 && devils > 50 }
];
...
let newAchievements = [
  ...
  { description: "Population Expert: Achieve grass population of 1500, pademelon population of 350, and devil population of 150", achieved: false, points: 400 }
];
...
function draw() {
  ...
  // Ecosystem Dynamics
  grass += (rainfall / 20 - invasiveSpecies) - (50 - rainfall) / 15; // Slower growth rate for grass
  pademelons += grass / 200 - devils / 15 - visitors - temperature / 10; // Slower growth for pademelons
  devils += pademelons / 80 - temperature / 15 - visitors / 6; // Slower growth for devils
...
function exportCSV() {
  ...
  csvContent += "Time,Grass,Pademelons,Devils\n";
  chart.data.labels.forEach((label, index) => {
    let row = `${label},${chart.data.datasets[0].data[index]},${chart.data.datasets[1].data[index]},${chart.data.datasets[2].data[index]}\n`;
    csvContent += row;
  });
...
function updatePoints() {
  ...
  ongoingObjectives.forEach(objective => {
    if (objective.condition(grass, pademelons, devils)) {
      objective.target--;
...
function checkEcosystemStability() {
  if (pademelonsHistory.length >= historyLength) {
    ...
    if (Math.abs(avgPademelons - 50) < 10 && Math.abs(avgDevils - 10) < 5) {
  ...
  newAchievements.forEach(achievement => {
    if (!achievement.achieved) {
      if (achievement.description === "Ecosystem Guardian: Achieve all ongoing objectives" && ongoingObjectives.every(obj => obj.achieved)) {
        ...
        alert(`Achievement Unlocked: ${achievement.description}\nPoints Awarded: ${achievement.points}`);
        displayAchievement(achievement.description);
      } else if (achievement.description === "Population Expert: Achieve grass population of 1500, pademelon population of 350, and devil population of 150" && grass >= 1500 && pademelons >= 350 && devils >= 150) {
        achievement.achieved = true;
        points += achievement.points;
        alert(`Achievement Unlocked: ${achievement.description}\nPoints Awarded: ${achievement.points}`);
        displayAchievement(achievement.description);
